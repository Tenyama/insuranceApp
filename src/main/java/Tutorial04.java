...
//        }
        ...
        // Create text boxes
        TextBox idTextBox = new TextBox(new TerminalSize(20,1));
        TextBox fullNameTextBox = new TextBox(new TerminalSize(20,1));
        TextBox cardTextBox = new TextBox(new TerminalSize(20,1));
        TextBox claimsTextBox = new TextBox(new TerminalSize(20,1));
        TextBox dependentTextBox = new TextBox(new TerminalSize(20,1));
        TextBox holderTextBox = new TextBox(new TerminalSize(20,1));

// Create labels for text boxes
        Label idLabel = new Label("ID:");
        Label fullNameLabel = new Label("Full Name:");
        Label cardLabel = new Label("Card:");
        Label claimsLabel = new Label("List of Claims:");
...
// Add components to a container (e.g., Panel)
        ...
        panel.addComponent(claimsLabel);
        panel.addComponent(claimsTextBox);
...
// Create a button to save the content of text boxes
        Button saveButton = new Button("Save", new Runnable() {
            ...
            public void run() {
                ...
                String claims = claimsTextBox.getText();
                String dependent = dependentTextBox.getText();
                String holder = holderTextBox.getText();

                // Now you can use these variables (id, fullName, card, claims, dependent, holder)
                // for further processing (e.g., saving to a file).
                panel.addComponent(new Label(id + fullName + card + claims + dependent + holder));
            }
        });
